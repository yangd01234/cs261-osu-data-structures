...
*/
//add heap taken from group work
void addHeap(DynArr *heap, TYPE node)
{
    assert(heap!=NULL);
    int parent;

    int pos = sizeDynArr(heap); //this is the next open spot in the array which is the size of the array
    addDynArr(heap,node); //this puts the value into the dynamic array location and will resize if necessary
    //printf("size: %d\n",heap->size);
    while(pos !=0){//this while loop will perculate up
        parent = (pos-1)/2;//we compute the parent  of the value we just inserted.  Which the parent is the middle of the array
        if(compare(getDynArr(heap,pos),getDynArr(heap,parent))==-1){//if the value at pos is less than the value at parent, it returns -1
            //if it is less than, we swap the two
            swapDynArr(heap,parent,pos);
            //then we repeat the process and update the current position
            pos = parent;
...
*/
void _adjustHeap(DynArr *heap, int max, int pos)//recursive function to check parent < child
{
    ...
    //from group
    if(rghtldx<max) {
        int minldx = _smallerIndexHeap(heap, leftldx, rghtldx);
        //if (getDynArr(heap, pos).priority > getDynArr(heap, minldx).priority) {
        if(compare(heap->data[pos],heap->data[minldx])==1){
            swapDynArr(heap,pos, minldx);
            _adjustHeap(heap, max, minldx);
        }
    }else if (leftldx<max) {
        if(compare(heap->data[pos],heap->data[leftldx])==1) {
            swapDynArr(heap,pos,leftldx);
            _adjustHeap(heap,max,leftldx);
        }
        return;
    }
...
{
    _buildHeap(heap);
    for(int max = sizeDynArr(heap)-1;max>=0;max--){
    //for(int max = 0;max>sizeDynArr(heap)-1;max++){
        //swap the zero position and the max
        swapDynArr(heap,0,max);

        //adjust the heap for the zero position and the max. TRICKLE
